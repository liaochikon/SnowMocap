...
# Config loading
config = Load_Config_Json("configs\snowmocap_default_config.json")
...
for i in tqdm(range(min_video_length), desc="Mocaping videos..."):
    ...
    td = []
    for h in triangulation_result['hrnet_triangulate_points']:
        td.append(h.tolist())
    test_data['test'].append(td)
...
save_blender_result(test_data, "test_data.json")