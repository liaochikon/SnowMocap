...
CameraGroup = sv.CameraGroup("snow_output_BA.txt")
...
for epoch in range(epochs):
    ...
    for i in range(length_array[0]):
        ...
        if CameraGroup.HRNet_2DPoint != []:
            ...
            for p, c in zip(CameraGroup.HRNet_2DPoint, CameraGroup.HRNet_Camera_Index):
                # p(nx2x20x2), c(nx2)
                c = np.array(c).ravel()
                p = np.array(p)
                p = p.reshape((p.shape[0] * p.shape[1], 20, 2))
                for part in range(20):
                    for c_ref in range(CameraGroup.Camera_Count):
    ...
    CameraGroup.HRNet_Bundle_Adjustment(np.array(W_list), np.array(w_list), np.array(c_list), np.array(p_list))
    cap_array[0].release()
    cap_array[1].release()
    cap_array[2].release()
    cap_array[3].release()

CameraGroup.OutputData("snow_output_BA")