...
parser.add_argument("config_path", help=": SnowMocap's fundamental config file", default="configs\snowmocap_default_config.json")
...
for i in tqdm(range(min_video_length), desc="Mocaping videos..."):
    try:
        ...
        triangulation_result['hrnet_triangulate_points'][0] = ukf.update(triangulation_result['hrnet_triangulate_points'][0])
        
        blender_result = Human_Triangulation_Blender(triangulation_result, blender_armature_profile)
        blender_result['blender_armature_control_points'] = bukf.update(blender_result['blender_armature_control_points'])
